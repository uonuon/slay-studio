...
export const store = {
  ...
  // Seed the demo menu/settings for a BRAND-NEW studio only. Hardened so it can
  // never overwrite a live menu (it once re-seeded the hardcoded defaults over
  // real services, wiping photos): runs at most once per database via a one-time
  // flag, and even then only *creates* docs that are genuinely absent.
  async ensureSeed() {
    if (!USE_FB) return;
    // (1) One-time guard. Once a studio is initialized we never seed again, so a
    //     transient empty/failed read on a later login can't trigger a reseed.
    const metaRef = doc(db, "config", "meta");
    try { if ((await getDoc(metaRef)).data()?.seeded) return; } catch (e) { return; } // read failed → do NOT seed

    // (2) Settings: create only if missing (never overwrite owner settings).
    const st = await getDoc(doc(db, "config", "settings"));
    if (!st.exists()) await setDoc(doc(db, "config", "settings"), DEFAULT_SETTINGS);

    // (3) Services: only when the collection is *genuinely* empty, and even then
    //     create-if-absent per doc so an existing service is never clobbered.
    const sv = await getDocs(collection(db, "services"));
    if (sv.empty) {
      for (const s of DEFAULT_SERVICES) {
        const ref = doc(db, "services", s.id);
        if (!(await getDoc(ref)).exists()) await setDoc(ref, s);
      }
    }

    // (4) Mark initialized → all future ensureSeed() calls short-circuit at (1).
  ...
  async getSettings() {
    // Merge over defaults so newly-added settings keys (location, promos,
    // colorsEnabled…) have sane values even on an older stored settings doc.
    let merged;
    if (USE_FB) { const d = await getDoc(doc(db, "config", "settings")); merged = { ...DEFAULT_SETTINGS, ...(d.exists() ? d.data() : {}) }; }
    else merged = { ...DEFAULT_SETTINGS, ...LS.get("settings", {}) };

    // One-time clip-ins palette upgrade: if the stored colors are still the
    // untouched v1 seed (6 basic shades, no photos/prices/additions), swap in
    // the v2 palette (full shade range + ombrés). Any owner edit — an added
    // color (uid id), an uploaded photo, a price, or a save that stamped
    // paletteV — opts out permanently.
    const V1_IDS = ["cc-black", "cc-dbrown", "cc-brown", "cc-caramel", "cc-blonde", "cc-burg"];
    const c = merged.clipins;
    if (c && (c.paletteV || 1) < (DEFAULT_SETTINGS.clipins.paletteV || 1)
        && (c.colors || []).every((x) => V1_IDS.includes(x.id) && !x.img && !+x.price)) {
      merged.clipins = { ...c, colors: DEFAULT_SETTINGS.clipins.colors, paletteV: DEFAULT_SETTINGS.clipins.paletteV };
    }
    return merged;
  },
  ...
  // ── materials / inventory (owner-only) ────────────────────────────────
  async listMaterials() {
    return USE_FB ? (await getDocs(collection(db, "materials"))).docs.map((d) => d.data()) : LS.get("materials", []);
  },
  async saveMaterial(m) {
    if (USE_FB) await setDoc(doc(db, "materials", m.id), m);
    else {
      const all = LS.get("materials", []);
      const i = all.findIndex((x) => x.id === m.id);
      if (i >= 0) all[i] = m; else all.push(m);
      LS.set("materials", all);
    }
  },
  async delMaterial(id) {
    if (USE_FB) await deleteDoc(doc(db, "materials", id));
    else LS.set("materials", LS.get("materials", []).filter((x) => x.id !== id));
  },
