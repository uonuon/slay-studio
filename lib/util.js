...
// ── promo codes (owner-defined % discounts) ───────────────────────────────
// Find an active promo by code (case/space-insensitive). Returns null if none.
export function findPromo(settings, code) {
  const c = (code || "").trim().toUpperCase();
  if (!c) return null;
  return (settings.promos || []).find(
    (p) => p.active && (p.code || "").trim().toUpperCase() === c
  ) || null;
}
// Apply a promo's percentage to a price (rounded EGP, never below 0).
export const applyPromo = (price, promo) =>
  promo ? Math.max(0, Math.round(price * (1 - (promo.pct || 0) / 100))) : price;
...
// True if a booking starting at `start` for `durMin` on `dateStr` overlaps an
// owner time-off block. The customer site can't reach blocked slots at all;
// this guards the dashboard's walk-in/edit forms so they respect the same
// blocks instead of silently booking through them.
export function isBlockedAt(settings, dateStr, start, durMin = 60) {
  if (isDayFullyBlocked(settings, dateStr)) return true;
  const s = t2m(start), e = s + Math.max(+durMin || 0, 1);
  return blockIntervals(settings, dateStr).some((iv) => s < iv.end && e > iv.start);
}
...
export function availableStarts(dateStr, durMin, settings, intervals) {
  ...
  // Minimum notice: today's slots must start at least `lead` minutes from now
  // (e.g. 90 → at 1:00 PM the first bookable slot is 2:30 PM). Owner-configurable.
  const lead = settings.leadTimeMins ?? 30;
  for (let s = open; s + durMin <= close; s += step) {
    const e = s + durMin;
    if (isToday && s < nowMin + lead) continue;
...
// Copy text to the clipboard, with a legacy fallback for older mobile browsers.
// Returns true on success so callers can toast the right thing.
